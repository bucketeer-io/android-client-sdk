...
internal class ApiClientImplRetryTest {
  ...
  @Test
  fun `getEvaluations - error 499 then 530 - fail immediately`() {
    ...
    assertThat(result).isInstanceOf(GetEvaluationsResult.Failure::class.java)
    val failure = result as GetEvaluationsResult.Failure
    assertThat(failure.error).isInstanceOf(BKTException.ClientClosedRequestException::class.java)
    assertThat(server.requestCount).isEqualTo(1)
  }

  @Test
  fun `registerEvents - error 499 then 530 - fail immediately`() {
    server.enqueue(mockClientClosedRequestResponse)
